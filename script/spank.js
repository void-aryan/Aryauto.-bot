...
module.exports.run = async function ({ api, event, args }) {
  ...
  const outputPath = __dirname + `/cache/spank_${userID}_${targetID}.png`;
  ...
  try {
    ...
    const spankURL = `https://api-canvass.vercel.app/spank?uid1=${userID}&uid2=${targetID}`;
    ...
    writer.on("finish", () => {
      api.sendMessage({
        body: formatFont(`${name1} has spanked ${name2}! 🍑`),
